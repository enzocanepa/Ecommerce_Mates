    ...
    return (<div className="flex min-h-screen bg-gray-100">
      ...
      <aside className="w-64 bg-[#4a5f2f] text-white flex flex-col shadow-xl flex-shrink-0">
        ...
        <nav className="flex-1 py-6 px-3 space-y-1">
          {navItems.map(({ to, label, icon: Icon, end }) => (<NavLink key={to} to={to} end={end} className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors text-sm ${isActive
        ...
        <div className="px-3 pb-6 space-y-1">
          <NavLink to="/" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-white/80 hover:bg-white/10 transition-colors">
            <ChevronRight className="w-5 h-5"/>
            Ver tienda
          </NavLink>
          <button onClick={handleSignOut} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-white/80 hover:bg-white/10 transition-colors">
