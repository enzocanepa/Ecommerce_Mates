...
type FormData = Omit<Product, 'id'> & { variantsInput: string };

function toFormData(p: Omit<Product, 'id'>): FormData {
  return { ...p, variantsInput: (p.variants ?? []).join(', ') };
}

function fromFormData(f: FormData): Omit<Product, 'id'> {
...
export function AdminProducts() {
  ...
  const [form, setForm] = useState<FormData>(toFormData(EMPTY_FORM));
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setError(
        'Error al subir imagen. Asegurate de haber creado el bucket "product-images" en Supabase Storage con acceso público.'
      );
    } finally {
      setUploading(false);
    }
  ...
  async function handleGenerateDescription() {
    if (!form.name.trim()) return setError('Ingresá el nombre del producto antes de generar la descripción.');
    if (!accessToken) return setError('La generación con IA requiere la Edge Function desplegada en Supabase (se activa en producción).');

    setGeneratingAI(true);
    setError('');
    try {
      const data = await apiRequest<{ short?: string; full?: string }>(
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={openCreate}
          className="flex items-center gap-2 bg-[#a8c95f] hover:bg-[#97b84f] text-[#4a5f2f] font-semibold px-4 py-2.5 rounded-lg transition-colors"
        >
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center justify-end gap-2">
                    <button
                      ...
                      className="p-1.5 text-gray-400 hover:text-[#4a5f2f] hover:bg-gray-100 rounded-lg transition-colors"
                      title="Editar"
                    >
            ...
            <div className="px-6 py-6 space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    ...
                    title={!accessToken ? 'Disponible cuando la Edge Function esté desplegada' : undefined}
                    className="flex items-center gap-1.5 text-xs text-[#4a5f2f] bg-[#c7e47d] hover:bg-[#b8d66e] px-2.5 py-1 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#a8c95f] mb-2"
