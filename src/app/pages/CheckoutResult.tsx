...
function decrementStock(items: any[]) {
  try {
    ...
    const updated = products.map((p: any) => {
      const ordered = items.find((i: any) => i.id === p.id);
      if (!ordered) return p;
...
export function CheckoutResult() {
  ...
  let resultType: ResultType = 'failure';
  ...
  useEffect(() => {
    ...
    const createOrder = async () => {
      const itemsToSave: any[] = cart.length > 0
        ? cart
    ...
    <div className="py-20 bg-gray-50 min-h-screen flex items-center justify-center">
      <div className="max-w-md w-full mx-4">
        <div className="bg-white rounded-2xl shadow-sm p-8 text-center">
          ...
          {paymentId && (
            <div className="bg-gray-50 rounded-lg px-4 py-3 text-left mb-6">
          ...
          <div className="flex flex-col gap-3">
            {resultType !== 'failure' ? (
              <Link
                to="/pedidos"
                className="flex items-center justify-center gap-2 bg-[#c7e47d] text-[#4a5f2f] py-3 rounded-xl font-semibold hover:bg-[#b8d66e] transition-colors"
              >
            ...
            <Link
              to="/tienda"
              className="text-[#6b8e3d] hover:text-[#4a5f2f] py-2 transition-colors text-sm"
            >
              Seguir comprando
            </Link>
          </div>
