...
export function ReviewsSection({ productId }: { productId: number }) {
  ...
  const fetchReviews = useCallback(async () => {
    ...
    } catch { /* silent — reviews are non-critical */ } finally {
      setLoading(false);
    }
  }, [productId]);
  ...
  return (
    <div className="mt-12 bg-white rounded-lg shadow-lg p-6 md:p-8">
      ...
      {user && !hasReviewed && (
        <form onSubmit={handleSubmit} className="bg-gray-50 rounded-xl p-5 mb-8 border border-gray-100">
          ...
          <button
            ...
            className="bg-[#a8c95f] hover:bg-[#97b84f] text-[#4a5f2f] font-semibold px-5 py-2 rounded-lg transition-colors disabled:opacity-50"
          >
      ...
      {!user && (
        <div className="bg-blue-50 border border-blue-100 text-blue-700 px-4 py-3 rounded-lg text-sm mb-8">
          <a href="/login" className="font-medium underline">Iniciá sesión</a> para dejar tu reseña.
        </div>
      )}

      {hasReviewed && (
        <div className="bg-green-50 border border-green-100 text-green-700 px-4 py-3 rounded-lg text-sm mb-8">
