...
export function AdminOrders() {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            if (accessToken) {
                try {
                    const data = await orderService.getAllOrders(accessToken);
    ...
    return (<div className="p-8">
      ...
      {loading ? (<div className="flex justify-center py-20">
        ...
        </div>) : (<div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {orders.map((order) => {
                ...
                return (<tr key={order.id} className="hover:bg-gray-50 transition-colors">
