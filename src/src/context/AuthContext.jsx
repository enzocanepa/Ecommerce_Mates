...
function buildUser(supabaseUser) {
    const role = supabaseUser.user_metadata?.role === 'admin' ? 'admin' : 'user';
    return {
        id: supabaseUser.id,
...
export function AuthProvider({ children }) {
    ...
    const signUp = async (email, password, name) => {
        try {
            ...
            if (!err?.__network && !isNetworkError(err))
                throw err;
            // ── Local fallback ──
            ...
            const sessionUser = { id: newUser.id, email, name, role: 'user' };
