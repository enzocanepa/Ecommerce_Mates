...
function NextArrow({ onClick }) {
    return (<button onClick={onClick} className="absolute right-4 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white text-[#6b8e3d] p-3 rounded-full shadow-lg transition-all hover:scale-110" aria-label="Siguiente">
      <ChevronRight className="w-6 h-6"/>
    </button>);
}
function PrevArrow({ onClick }) {
    return (<button onClick={onClick} className="absolute left-4 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white text-[#6b8e3d] p-3 rounded-full shadow-lg transition-all hover:scale-110" aria-label="Anterior">
...
const sliderSettings = {
    ...
    appendDots: (dots) => (<ul className="flex justify-center gap-2">
      {dots.map((dot, index) => (<li key={index} className={dot.props.className}>
          {dot.props.children}
        </li>))}
    </ul>),
    customPaging: () => (<button className="w-3 h-3 rounded-full bg-white/60 hover:bg-white transition-all"/>),
};
export function HeroCarousel() {
    return (<section className="relative w-full">
      <style>{`
        ...
        .slick-dots li.slick-active button { background-color: white; transform: scale(1.3); }
      `}</style>
      <Slider {...sliderSettings}>
        {carouselImages.map((image, index) => (<div key={index} className="relative">
            <div className="w-full h-[300px] sm:h-[400px] md:h-[500px] lg:h-[600px] bg-gray-100">
              <img src={image} alt={`Slide ${index + 1} - Mates Aconcagua`} className="w-full h-full object-contain" style={{ objectPosition: 'center' }}/>
            </div>
          </div>))}
      </Slider>
    </section>);
